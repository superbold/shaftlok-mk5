...
2. The public pay page shows quoted total vs card total (3% added **here**, not by Stripe) and two buttons: bank transfer / card.
...
5. Env: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, optional `NUXT_PUBLIC_SITE_URL` (defaults to `https://shaftlok.com`). Dashboard: enable Cards, Bank transfers, and ACH; add the live webhook URL; turn on Radar.
